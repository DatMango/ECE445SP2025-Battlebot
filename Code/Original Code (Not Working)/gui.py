...
from link import WiFiCommunicator, OutMessage
...
class GUI(tk.Tk):
    ...
    def __create_led_button(self):
        ...
        for i in range(5):
            btn_state_txt = tk.StringVar(value=self.ON_BUTTON_STR)
            btn = tk.Button(self, textvariable=btn_state_txt, bg=self.ON_COLOR)
            
            # Store button state and button reference
            self._btn_states.append(btn_state_txt)
            self._buttons.append(btn)
            
            # Use functools.partial to bind the index instead of passing state directly
            btn.config(command=functools.partial(self.__on_led_btn_click_cb, i))
            
            btn.place(relx=0.1 + i * 0.18, rely=0.2, relwidth=0.15)
    ...
    def __on_led_btn_click_cb(self, index): #*args):
        '''
        '''
        ...
        btn_state_txt = self._btn_states[index]
        btn = self._buttons[index]
        
        led_on = btn_state_txt.get() == self.OFF_BUTTON_STR
        btn_state_txt.set(self.ON_BUTTON_STR if led_on else self.OFF_BUTTON_STR)
        btn['bg'] = self.ON_COLOR if led_on else self.OFF_COLOR

        # Send corresponding message
        # msg = OutMessage(data='f' if led_on else 'n')
        # self._communicator.send_message(msg)
        print(index)
        msg = OutMessage(data=f"{index}{'f' if led_on else 'n'}")  # Include button index in message
        print(index)
        print(msg)
        self._communicator.send_message(msg)
